/*
 * Highcharts JS v3.0.1 (2013-04-09) Exporting module
 * 
...
(function(e) {
	...
	contextPath = contextPath.substring(0,contextPath.indexOf('main'));
	...
	p(s.lang, {
				printChart : "打印图表",
				downloadPNG : "下载为PNG格式",
				downloadJPEG : "下载为JPEG格式",
				downloadPDF : "下载为PDF格式",
				downloadSVG : "下载为SVG格式",
	...
	s.exporting = {
		type : "image/png",
		//url : contextPath+"main/chart/export/",
		url : '/main/chart/export/',
		buttons : {
			contextButton : {
				symbol : "menu",
	...
	p(y.prototype, {
		getSVG : function(a) {
			...
			d = f.exporting.sourceWidth || f.chart.width || /px$/.test(d)
					&& parseInt(d, 10) || 600;
			g = f.exporting.sourceHeight || f.chart.height || /px$/.test(g)
					&& parseInt(g, 10) || 400;
			p(f.chart, {
						animation : !1,
						renderTo : a,
			...
			c.destroy();
			j(a);
			d = d.replace(/zIndex="[^"]+"/g, "").replace(/isShadow="[^"]+"/g,
					"").replace(/symbolName="[^"]+"/g, "").replace(
					/jQuery[0-9]+="[^"]+"/g, "").replace(/url\([^#]+#/g,
					"url(#").replace(/<svg /,
					'<svg xmlns:xlink="http://www.w3.org/1999/xlink" ')
					.replace(/ href=/g, " xlink:href=").replace(/\n/, " ")
					.replace(/<\/svg>.*?$/, "</svg>").replace(/&nbsp;/g, " ")
					.replace(/&shy;/g, "­").replace(/<IMG /g, "<image ")
					.replace(/height=([^" ]+)/g, 'height="$1"').replace(
							/width=([^" ]+)/g, 'width="$1"').replace(
							/hc-svg-href="([^"]+)">/g, 'xlink:href="$1"/>')
			...
			d = d.replace(/(url\(#highcharts-[0-9]+)&quot;/g, "$1").replace(
					/&quot;/g, "'");
		...
		exportChart : function(a, b) {
			var a = a || {}, c = this.options.exporting, c = this.getSVG(k({
						chart : {
							borderRadius : 0
						}
					}, c, b, {
			...
			var form = document.createElement('form')
				form.action = a.url;
				form.method = 'post';
			var filename = a.filename || 'chart';
			var width = a.width || 0;
			var scale = a.scale || 2;
			var pps = ['filename','type','width','scale','svg'];
			var vvs = [filename,a.type,width,scale,c]
			for(var i = 0;i < pps.length;i++){
				var input = document.createElement('input');
					input.setAttribute('name',pps[i]);
					input.setAttribute('type','hidden');
					input.setAttribute('value',vvs[i]);
		...
		addButton : function(a) {
			...
			if (a.enabled !== !1) {
				...
				d ? j = function() {
					...
					b.contextMenu("contextmenu", i, f.translateX, f.translateY,
							f.width, f.height, f);
					f.setState(2)
				});
				a.text && a.symbol
		...
		destroyExport : function(a) {
			...
			for (b = 0; b < a.exportSVGElements.length; b++)
				c = a.exportSVGElements[b], c.onclick = c.ontouchstart = null, a.exportSVGElements[b] = c
						.destroy();
			for (b = 0; b < a.exportDivElements.length; b++)
				c = a.exportDivElements[b], B(c, "mouseleave"), a.exportDivElements[b] = c.onmouseout = c.onmouseover = c.ontouchstart = c.onclick = null, j(c)
		}
	});
